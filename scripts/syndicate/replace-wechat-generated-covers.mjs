...
for (const file of files) {
  ...
  if (article.cover.endsWith("/wechat-cover.png")) articles.push(article);
}
...
for (const draft of existingDrafts) {
  for (const title of draft.titles) {
    const items = draftsByTitle.get(title) || [];
    items.push(draft.mediaId);
    draftsByTitle.set(title, items);
  }
}

const summary = { total: articles.length, replaced: 0, failed: 0, deleted: 0 };
console.log(`准备替换 ${articles.length} 篇使用新生成封面的公众号草稿。`);
